...
async function testCookieMode() {
  ...
  assert(cookies.some(c => c.includes('refreshToken')), 'refresh token cookie should be set');
  assert(cookies.some(c => c.includes('csrfToken')), 'CSRF token cookie should be set');
...
async function testAdapterCompatibility() {
  ...
  for (const method of adapterMethods) {
    assert(typeof memoryAuth.adapter[method] === 'function', 
      `Memory adapter should have ${method}`);
    assert(typeof fileAuth.adapter[method] === 'function', 
      `File adapter should have ${method}`);
  }
...
try {
  ...
  // Run tests without Express integration
