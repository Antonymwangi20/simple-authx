/**
 * @fileoverview Simple-AuthX - Minimal Express authentication helper using JWT
 * @module simple-authx
 * @version 2.0.6
 ...
 * @param {string} [config.accessExpiresIn='1h'] - Access token expiration time
...
export default function AuthX(config = {}) {
  ...
  const userStore = config.userStore || {
    _map: new Map(),
    async get(username) {
      return this._map.get(username) || null;
    },
    async set(username, user) {
      this._map.set(username, user);
      return user;
    },
  };
  ...
  function loginHandler(getUserFn) {
    ...
    return async (req, res) => {
      ...
      const tokens = await issueTokens(res, username);
      return res.json({ message: 'Login successful', accessToken: tokens.accessToken });
    };
  }
  ...
  async function refreshHandler(req, res) {
    ...
    if (!rotated)
      return res.status(403).json({ error: 'Token reuse detected. Re-login required.' });
