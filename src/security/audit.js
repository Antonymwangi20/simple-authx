/**
 * @fileoverview Audit logger for tracking authentication events and security incidents
...
/**
 * Audit logger for tracking and analyzing authentication events
 *
 * @class AuditLogger
 * @example
...
export class AuditLogger {
   ...
   * @param {string} [config.level='info'] - Log level (trace, debug, info, warn, error, fatal)
   * @param {Object} [config.store] - Storage adapter for persistent audit logs
   * @param {Object} [config.pinoOptions] - Additional pino logger options
   */
  constructor(config = {}) {
    this.logger = pino({
      name: 'authx-audit',
  ...
  async getSecurityEvents(severity = 'high', timeframe = '24h') {
    ...
    const timeframeMs =
      typeof timeframe === 'string' ? ms(/** @type {any} */ (timeframe)) : timeframe;
    const events = await this.store.audit.findSecurityEvents({
      severity,
      since: new Date(Date.now() - Number(timeframeMs)),
    });
  ...
  async getLoginStats(timeframe = '24h') {
    ...
    const timeframeMs =
      typeof timeframe === 'string' ? ms(/** @type {any} */ (timeframe)) : timeframe;
    const since = new Date(Date.now() - Number(timeframeMs));
  ...
  async getSecurityInsights() {
    ...
    const now = new Date();
    const weekAgo = new Date(now.getTime() - ms('7d'));
